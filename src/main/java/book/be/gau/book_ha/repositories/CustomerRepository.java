...
public interface CustomerRepository extends JpaRepository<Customer, Integer> {
  ...
  @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM Customer c WHERE c.customer_staff_email = ?1")
  boolean existsByCustomerStaffEmail(String customerStaffEmail);
}